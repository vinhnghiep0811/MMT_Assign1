...
class Peer:
    def __init__(self, peer_id, peer_port, tracker_ip, tracker_port):
        ...
        self.connected_peers = []  # Danh sách lưu thông tin các peer đang kết nối

    def generate_file_hash(self, file_path):
        """Tạo hash từ nội dung file để dùng làm magnet link."""
    ...
    def upload_file(self, file_name):
        """Gửi yêu cầu upload file đến tracker."""
        ...
        # Tạo kết nối đến tracker và gửi yêu cầu upload
    ...
    def register_with_tracker(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            # Cập nhật danh sách các peer sau khi đăng ký
            self.update_connected_peers()

    def update_connected_peers(self):
        """Yêu cầu danh sách các peer từ tracker và cập nhật."""
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((self.tracker_ip, self.tracker_port))
            message = f"UPDATE"
            s.send(message.encode())
    ...
    def request_file(self, hash_code):
        """Yêu cầu danh sách các peer có file với hash_code từ tracker."""
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((self.tracker_ip, self.tracker_port))
    ...
    def show_files(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            response = s.recv(4096).decode().strip()  # 4096 bytes to handle larger responses
...
def main():
    ...
    print("Peer initialized. Please type your command (register, show, upload, exit):")
